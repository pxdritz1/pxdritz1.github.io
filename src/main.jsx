...
const projects = [
  [
    "https://cdn.modrinth.com/data/zLOxi2dV/8156284569e35a77d98e6a0730579df2ca175868_96.webp",
    "blokkusus",
    "126+ new building blocks for minecraft, made to sit naturally beside vanilla.",
    "modrinth.com/project/blokkusus",
  ],
  [
    "https://cdn.modrinth.com/data/mUSAnHRV/e5a283057d3b1573f722cad5fb34f15f42134d96_96.webp",
    "mercurizer",
    "a sodium addon for low-end hardware, tuned for a smoother little world.",
    "modrinth.com/project/mercurizer",
  ],
  [
    "https://cdn.modrinth.com/data/sSJDIG90/ddd4d49b3a52c8bad5851b72a4504c295f4da8de_96.webp",
    "potato optimizer",
    "a lightweight modpack for high fps with sodium, lithium, and friends.",
    "modrinth.com/modpack/potatooptimizer",
  ],
];
const navItems = [
  ["home", "home"],
...
function Avatar({ src, className = "" }) {
  return (
    <img
      className={`m3-outline ${className}`}
...
function ThemeToggle({ dark, setDark }) {
  ...
  return (
    <button
      className="hover-surface rounded-xl p-2 text-ink"
      onClick={toggleTheme}
...
function Header({ avatar, dark, setDark }) {
  ...
  return (
    <header className="sticky top-0 z-30 px-4 pt-4">
      <nav
        className="mx-auto flex max-w-6xl items-center justify-between rounded-2xl border border-ink/10 bg-paper/80 px-3 py-2 shadow-soft backdrop-blur-md"
        aria-label="main navigation"
      >
        <a
          href="#home"
          className="hover-surface flex items-center gap-2 rounded-xl px-2 py-1.5 font-display text-sm font-semibold tracking-tight text-ink"
        >
          <Avatar
            src={avatar}
            className="h-7 w-7 rounded-lg object-cover ring-2 ring-coral/20"
          />
          <span>
            pxdritz<span className="text-coral">.</span>
          </span>
        </a>
        <button
        ...
        <div
          className={`${open ? "flex" : "hidden"} absolute left-4 right-4 top-[4.5rem] flex-col gap-1 rounded-2xl border border-ink/10 bg-paper p-2 shadow-soft md:static md:flex md:flex-row md:items-center md:border-0 md:bg-transparent md:p-0 md:shadow-none`}
        >
          {navItems.slice(1).map(([path, label]) => (
            <a
              ...
              className={`hover-surface rounded-xl px-3 py-2 text-sm text-ink/65 hover:text-ink ${window.location.hash === `#${path}` ? "bg-ink text-paper hover:bg-ink hover:text-paper" : ""}`}
            >
              {label}
            </a>
          ))}
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="ml-1 inline-flex items-center gap-2 rounded-xl bg-coral px-3 py-2 text-sm font-semibold text-white hover:bg-coral-dark"
          >
...
function Footer({ avatar }) {
  return (
    <footer className="mx-auto mt-20 flex max-w-6xl flex-col gap-4 border-t border-ink/10 px-4 py-8 text-sm text-ink/50 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <Avatar src={avatar} className="h-8 w-8 rounded-full object-cover" />
      </div>
...
function PageIntro({ eyebrow, title, copy }) {
  ...
  return (
    <div
      ref={ref}
      className={`${visible ? "reveal-visible" : "reveal-hidden"} mb-10 max-w-2xl`}
    >
      <p className="mb-3 font-mono text-xs uppercase tracking-[0.22em] text-coral">
        {eyebrow}
      </p>
      <h1 className="font-display text-5xl font-bold leading-[0.95] tracking-[-0.06em] text-ink sm:text-7xl">
        {title}
      </h1>
      <p className="mt-5 max-w-xl text-lg leading-8 text-ink/60">{copy}</p>
    </div>
  );
}
  ...
  return (
    <main className="page-enter mx-auto max-w-6xl px-4 pb-4 pt-20">
      <div className="grid items-end gap-12 lg:grid-cols-[1.2fr_.8fr] lg:pt-14">
        <div>
          <div className="mb-7 flex items-center gap-3 text-sm text-ink/50">
            <span className="h-2 w-2 rounded-full bg-mint" /> currently building
            things & making art
          </div>
          <h1 className="max-w-4xl font-display text-6xl font-bold leading-[0.9] tracking-[-0.075em] text-ink sm:text-8xl">
            soft ideas,
            <br />
            <span className="text-coral">sharp tools.</span>
          </h1>
          <p className="mt-8 max-w-xl text-lg leading-8 text-ink/60">
            developer and artist making minecraft mods, tiny tools, and
            wallpapers with a quiet amount of care.
          </p>
          <div className="mt-9 flex flex-wrap gap-3">
            <a
              href="#projects"
              className="inline-flex items-center gap-2 rounded-xl bg-ink px-5 py-3 text-sm font-semibold text-paper hover:bg-coral"
            >
...
function KofiWidget() {
  ...
  useEffect(() => {
    const initializeWidget = () => {
      ...
      window.kofiwidget2.init("Support me on Ko-fi", "#e96858", "A6A42663NV");
...
function SpotifyWidget() {
  return (
    <section
      className="rounded-[2rem] border border-ink/10 bg-white/60 p-4 shadow-soft"
    ...
    >
      <div className="mb-3 flex items-center justify-between px-1">
        <div>
          ...
          <h2 className="font-display text-lg font-bold text-ink">
            a little song for the road
          </h2>
        </div>
        <a
          href="https://open.spotify.com/intl-pt/track/6xUrOmjUxQDd2QLtwtxSqg"
          target="_blank"
          rel="noreferrer"
          className="hover-surface rounded-xl p-2 text-ink"
      ...
      <iframe
        ...
        src="https://open.spotify.com/embed/track/6xUrOmjUxQDd2QLtwtxSqg?utm_source=generator&theme=0"
...
function ProjectCard({ project, type }) {
  ...
  const description = isGithub
    ? project.description || "a github project by pxdritz1."
    : project.description || "a project published on modrinth.";
  const link = isGithub
    ? project.html_url
    : `https://modrinth.com/project/${project.slug}`;
    ...
    <article className="card-enter group flex flex-col rounded-3xl border border-ink/10 bg-white/60 p-5 shadow-soft transition-transform hover:-translate-y-1">
        ...
        <div className="mt-5 border-t border-ink/10 pt-4">
          ...
          <ul className="space-y-1.5">
            {project.commits.map((commit) => (
              <li key={commit.sha} className="truncate text-xs text-ink/50">
      ...
      <div className="mt-7 flex items-center justify-between border-t border-ink/10 pt-4 text-xs">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center gap-1 font-semibold text-coral"
        >
    ...
    return (
      <main className="page-enter mx-auto max-w-6xl px-4 pb-4 pt-20">
        <PageIntro
          ...
          title="projects, but make them useful."
          copy="loading projects and recent commits..."
        />
        <div className="grid gap-4 lg:grid-cols-3">
          {Array.from({ length: 6 }, (_, index) => (
            <div
              key={index}
              className="h-64 animate-pulse rounded-3xl bg-ink/5"
            />
          ))}
        </div>
      </main>
  ...
  return (
    <main className="page-enter mx-auto max-w-6xl px-4 pb-4 pt-20">
      <PageIntro
        eyebrow="selected work"
        title="projects, but make them useful."
        copy="everything published on modrinth and github, kept fresh through their public apis."
      />
      {data.error && (
        <p className="mb-6 rounded-2xl bg-coral/10 p-4 text-sm text-ink/60">
      ...
      <section>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {data.modrinth.map((project) => (
            <ProjectCard key={project.id} project={project} type="modrinth" />
          ))}
        </div>
      </section>
      <section className="mt-12">
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {data.github.map((project) => (
            <ProjectCard key={project.id} project={project} type="github" />
          ))}
        </div>
      </section>
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        <SmallProject
          ...
          title="edonme studios"
          copy="coordination, process, and digital project delivery."
          link="https://edonme.dev"
        />
        <SmallProject
          icon={<Code2 size={20} />}
          title="tools & scripts"
          copy="python utilities and automation for better workflows."
  ...
  return (
    <main className="page-enter mx-auto max-w-6xl px-4 pb-4 pt-20">
      <PageIntro
        eyebrow="selected work"
        title="projects, but make them useful."
        copy="small tools and minecraft things, built for people who like their games cozy and their fps high."
      />
      <div className="grid gap-4 lg:grid-cols-3">
        {projects.map(([icon, title, copy, link], index) => (
          <article
            key={title}
          ...
          >
            ...
            <div className="mt-7 flex items-center justify-between border-t border-ink/10 pt-4 text-xs">
              <span className="rounded-full bg-mint/30 px-2.5 py-1 text-ink/65">
                fabric / modrinth
              </span>
              <a
                href={`https://${link}`}
...
function SmallProject({ icon, title, copy, link }) {
  return (
    <a
      href={link}
      target="_blank"
      rel="noreferrer"
      className="flex items-center gap-4 rounded-3xl border border-ink/10 bg-yellow/25 p-5 hover-surface"
    >
      <span className="grid h-11 w-11 place-items-center rounded-2xl bg-yellow text-ink">
        {icon}
      </span>
      <span className="flex-1">
        <strong className="font-display text-lg text-ink">{title}</strong>
        <span className="mt-1 block text-sm text-ink/50">{copy}</span>
      </span>
  ...
  return (
    <main className="page-enter mx-auto max-w-6xl px-4 pb-4 pt-20">
      <PageIntro
        ...
        title="a little gallery of things."
        copy="wallpapers, commissions, and experiments. click around, there are nice pixels hiding here."
      />
      <div className="columns-1 gap-4 sm:columns-2">
        {galleryItems
          .filter(([src]) => !src.endsWith("/cujos.png"))
          .map(([src, title, type]) => (
            <button
              key={src}
              onClick={() => {
            ...
            >
              ...
              />
              <span className="absolute inset-x-0 bottom-0 flex items-end justify-between bg-gradient-to-t from-ink/70 to-transparent px-5 pb-5 pt-14 text-white">
                <span>
                  <strong className="block font-display text-xl">
                    {title}
                  </strong>
                  <small className="text-white/65">{type}</small>
                </span>
                <ImageIcon size={18} />
              </span>
            </button>
...
function About({ avatar }) {
  return (
    <main className="mx-auto max-w-6xl px-4 pb-4 pt-20">
      <PageIntro
      ...
      <div className="grid gap-5 lg:grid-cols-[.7fr_1.3fr]">
        <div className="grid content-start gap-5">
          ...
          <div className="mt-8 border-t border-ink/10 pt-6 text-sm leading-6 text-ink/65">
            ...
            <p className="mt-4">
              i consider myself a creative person who is always thinking
              outside the box.
            </p>
          </div>
          </div>
        ...
        <div className="rounded-3xl border border-ink/10 bg-white/60 p-6 sm:p-8">
          ...
          <div className="mt-8 rounded-2xl bg-mint/20 p-4">
            <p className="font-mono text-xs uppercase tracking-[0.18em] text-coral">
              minecraft pvp era
            </p>
            <p className="mt-2 text-sm leading-6 text-ink/65">
              i was a frequent minecraft 1.8.9 pvp player, known on NameMC as{" "}
              <strong className="text-ink">pxdritz</strong>.
            </p>
            <a
              href="https://namemc.com/profile/pxdritz"
              target="_blank"
              rel="noreferrer"
              className="mt-3 inline-flex items-center gap-1 text-sm font-semibold text-coral"
            >
              view NameMC <ExternalLink size={13} />
            </a>
          </div>
          <div className="mt-8">
            ...
            <div className="flex flex-wrap items-center gap-3 rounded-2xl border border-ink/10 bg-paper p-4">
              {languages.map(([name, link]) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="hover-surface rounded-xl p-1 transition hover:-translate-y-1"
...
function Contact() {
  const links = [
    [
      "discord",
      "pxotitas",
      "chat, commissions, friendly hellos",
      <MessageCircle size={21} />,
      "https://discord.com/users/pxotitas",
    ],
    [
      "github",
      "pxdritz1",
      "code, experiments, and public things",
      <Code2 size={21} />,
    ...
    [
      ...
      "mods and modpacks",
      <Code2 size={21} />,
      "https://modrinth.com/user/pxotitas",
    ],
    [
      "ko-fi",
      "pxdritz1",
      "support the little studio",
      <Sparkles size={21} />,
    ...
    [
      "server",
      "px's server",
      "a brazilian community for friends and minecraft",
      <MessageCircle size={21} />,
      "https://discord.gg/fMkj87H2ds",
    ],
    [
      "server",
      "code's café",
      "a brazilian programming and tech community",
      <MessageCircle size={21} />,
      "https://discord.gg/BDKNDHvjrD",
    ],
    [
      "server",
      "servidor dos programadores",
      "a brazilian server for people who code",
      <MessageCircle size={21} />,
      "https://discord.gg/programador",
    ],
  ];
  return (
    <main className="mx-auto max-w-6xl px-4 pb-4 pt-20">
      <PageIntro
        eyebrow="say hello"
        title="let's make something nice."
        copy="for commissions, collaborations, or just a friendly wave, discord is the fastest way to find me."
      />
      <div className="grid gap-3 sm:grid-cols-2">
        {links.map(([name, handle, copy, icon, link], index) => (
            <a
              key={`${name}-${handle}-${index}`}
              href={link}
              target="_blank"
              rel="noreferrer"
              className="group flex items-center gap-4 rounded-3xl border border-ink/10 bg-white/60 p-5 hover-surface hover:border-coral"
          >
            ...
            <ArrowUpRight
              size={18}
              className="text-ink/35 transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5"
            />
          </a>
        ))}
      </div>
      <div className="mt-8 flex items-start gap-3 rounded-3xl border border-dashed border-coral/35 bg-coral/5 p-5 text-sm leading-6 text-ink/55">
        <MessageCircle size={18} className="mt-1 shrink-0 text-coral" />
        <span>
          all three servers above are brazilian communities. portuguese is the
          main language, and most people can also speak english.
        </span>
      </div>
      <div className="mt-4 flex items-center gap-3 rounded-3xl border border-dashed border-coral/35 bg-coral/5 p-5 text-sm text-ink/55">
        <Mail size={18} className="text-coral" /> usually replies within a few
        hours, give or take a snack break.
      </div>
    </main>
  );
}
  ...
  return (
    <div className="min-h-screen overflow-hidden">
      <div className="pointer-events-none fixed inset-0 -z-10 bg-paper">
        <div className="absolute -left-32 top-40 h-80 w-80 rounded-full bg-yellow/30 blur-3xl dark:hidden" />
        <div className="absolute -right-24 top-[28rem] h-96 w-96 rounded-full bg-coral/10 blur-3xl dark:hidden" />
      </div>
      <Header avatar={avatar} dark={dark} setDark={setDark} />
      {page}
